...
let config: ApolloConfig | null = null;

export function setConfig(newConfig: ApolloConfig): void {
  config = newConfig;
}

export function getConfig(): ApolloConfig | null {
  return config;
}
...
export function formatPaginatedResponse<T>(
  ...
  const items = (data[itemKey as keyof typeof data] as T[]) || [];
...
export function formatPerson(person: Record<string, unknown>): Record<string, unknown> {
  return {
    id: person.id,
    ...
    organizationName: (person.organization as Record<string, unknown>)?.name,
...
export function formatAccount(account: Record<string, unknown>): Record<string, unknown> {
  return {
    id: account.id,
    name: account.name,
...
export function formatSequence(sequence: Record<string, unknown>): Record<string, unknown> {
  return {
    id: sequence.id,
    name: sequence.name,
