...
import { setConfig, getConfig, makeApiRequest, formatPerson, formatOrganization, formatContact, formatAccount, formatSequence, } from "./services/apollo-client.js";
import { analyzeVacancy, formatAnalysisForPipedrive } from "./services/vacancy-analyzer.js";
import { VacancyAnalysisSchema, ConfigureSchema, PersonEnrichmentSchema, BulkPersonEnrichmentSchema, OrganizationEnrichmentSchema, PeopleSearchSchema, OrganizationSearchSchema, CreateContactSchema, UpdateContactSchema, SearchContactsSchema, CreateAccountSchema, UpdateAccountSchema, SearchAccountsSchema, SearchSequencesSchema, AddContactsToSequenceSchema, CreateTaskSchema, SearchTasksSchema, CreateDealSchema, ListDealsSchema, GetApiUsageSchema, ListUsersSchema, ListEmailAccountsSchema, } from "./schemas/index.js";
...
// CONFIGURATION TOOL
// ============================================================
server.registerTool("apollo_configure", {
...
// ============================================================
server.registerTool("apollo_analyze_vacancy", {
...
Returns:
  Comprehensive analysis with score, improvements, and optimized text.`,
    ...
    const result = await analyzeVacancy(params.vacancyText, params.companyName, params.jobTitle);
    if (!result.success) {
        return {
            content: [{
                    type: "text",
...
// ENRICHMENT TOOLS
// ============================================================
server.registerTool("apollo_enrich_person", {
...
Note: Uses API credits. Requires email OR linkedinUrl OR (firstName+lastName+domain).`,
    ...
    const response = await makeApiRequest("/people/match", "POST", undefined, queryParams);
    if (!response.success) {
        return {
...
server.registerTool("apollo_enrich_person_bulk", {
...
Note: Uses API credits for each successful match.`,
    ...
    const response = await makeApiRequest("/people/bulk_match", "POST", {
        details,
        reveal_personal_emails: params.revealPersonalEmails,
        reveal_phone_number: params.revealPhoneNumber,
    });
...
server.registerTool("apollo_enrich_organization", {
...
Returns:
  Organization data including industry, size, funding, technologies, etc.`,
    ...
    const response = await makeApiRequest("/organizations/enrich", "GET", undefined, { domain: params.domain });
    if (!response.success) {
        return {
            content: [{ type: "text", text: `Error: ${response.error}` }],
        };
...
// SEARCH TOOLS
// ============================================================
server.registerTool("apollo_search_people", {
...
Returns:
  Paginated list of matching people with professional details.`,
    ...
    if (params.personSeniorities)
        body.person_seniorities = params.personSeniorities;
    if (params.qOrganizationDomains)
        body.q_organization_domains = params.qOrganizationDomains;
    if (params.organizationLocations)
        body.organization_locations = params.organizationLocations;
    if (params.organizationNumEmployeesRanges)
        body.organization_num_employees_ranges = params.organizationNumEmployeesRanges;
    if (params.organizationIndustryTagIds)
        body.organization_industry_tag_ids = params.organizationIndustryTagIds;
    if (params.qKeywords)
        body.q_keywords = params.qKeywords;
    const response = await makeApiRequest("/mixed_people/search", "POST", body);
    if (!response.success) {
        return {
            content: [{ type: "text", text: `Error: ${response.error}` }],
        };
...
server.registerTool("apollo_search_organizations", {
...
Returns:
  Paginated list of matching organizations.`,
    ...
    if (params.qOrganizationName)
        body.q_organization_name = params.qOrganizationName;
    if (params.organizationLocations)
        body.organization_locations = params.organizationLocations;
    if (params.organizationNumEmployeesRanges)
        body.organization_num_employees_ranges = params.organizationNumEmployeesRanges;
    ...
    if (params.qKeywords)
        body.q_keywords = params.qKeywords;
    const response = await makeApiRequest("/mixed_companies/search", "POST", body);
    if (!response.success) {
        return {
            content: [{ type: "text", text: `Error: ${response.error}` }],
        };
...
// CONTACT TOOLS
// ============================================================
server.registerTool("apollo_create_contact", {
...
Returns:
  Created contact data with ID.`,
    ...
    if (params.websiteUrl)
        body.website_url = params.websiteUrl;
    if (params.accountId)
        body.account_id = params.accountId;
    if (params.ownerId)
        body.owner_id = params.ownerId;
    if (params.phone)
        body.direct_phone = params.phone;
    if (params.linkedinUrl)
        body.linkedin_url = params.linkedinUrl;
    if (params.labelIds)
        body.label_ids = params.labelIds;
    const response = await makeApiRequest("/contacts", "POST", body);
    if (!response.success) {
        return {
            content: [{ type: "text", text: `Error: ${response.error}` }],
        };
...
server.registerTool("apollo_update_contact", {
...
Returns:
  Updated contact data.`,
    ...
    if (params.labelIds)
        body.label_ids = params.labelIds;
    const response = await makeApiRequest(`/contacts/${params.contactId}`, "PUT", body);
    if (!response.success) {
        return {
            content: [{ type: "text", text: `Error: ${response.error}` }],
        };
...
server.registerTool("apollo_search_contacts", {
...
Returns:
  Paginated list of your contacts.`,
    ...
    if (params.qKeywords)
        body.q_keywords = params.qKeywords;
    if (params.contactLabelIds)
        body.contact_label_ids = params.contactLabelIds;
    if (params.accountIds)
        body.account_ids = params.accountIds;
    if (params.ownerIds)
        body.owner_ids = params.ownerIds;
    const response = await makeApiRequest("/contacts/search", "POST", body);
    if (!response.success) {
        return {
            content: [{ type: "text", text: `Error: ${response.error}` }],
        };
...
// ACCOUNT TOOLS
// ============================================================
server.registerTool("apollo_create_account", {
...
Returns:
  Created account data with ID.`,
    ...
    if (params.labelIds)
        body.label_ids = params.labelIds;
    const response = await makeApiRequest("/accounts", "POST", body);
    if (!response.success) {
        return {
            content: [{ type: "text", text: `Error: ${response.error}` }],
        };
...
server.registerTool("apollo_update_account", {
...
Returns:
  Updated account data.`,
    ...
    if (params.ownerId)
        body.owner_id = params.ownerId;
    const response = await makeApiRequest(`/accounts/${params.accountId}`, "PATCH", body);
    if (!response.success) {
        return {
            content: [{ type: "text", text: `Error: ${response.error}` }],
        };
...
server.registerTool("apollo_search_accounts", {
...
Returns:
  Paginated list of your accounts.`,
    ...
    if (params.qKeywords)
        body.q_keywords = params.qKeywords;
    if (params.accountLabelIds)
        body.account_label_ids = params.accountLabelIds;
    if (params.ownerIds)
        body.owner_ids = params.ownerIds;
    const response = await makeApiRequest("/accounts/search", "POST", body);
    if (!response.success) {
        return {
            content: [{ type: "text", text: `Error: ${response.error}` }],
        };
...
// SEQUENCE TOOLS
// ============================================================
server.registerTool("apollo_search_sequences", {
...
Returns:
  List of sequences with performance metrics.`,
    ...
    if (params.activeOnly)
        body.active = true;
    const response = await makeApiRequest("/emailer_campaigns/search", "POST", body);
    if (!response.success) {
        return {
            content: [{ type: "text", text: `Error: ${response.error}` }],
        };
...
server.registerTool("apollo_add_contacts_to_sequence", {
...
Returns:
  Confirmation of contacts added.`,
    ...
    const response = await makeApiRequest(`/emailer_campaigns/${params.sequenceId}/add_contact_ids`, "POST", body);
    if (!response.success) {
        return {
            content: [{ type: "text", text: `Error: ${response.error}` }],
        };
...
// TASK TOOLS
// ============================================================
server.registerTool("apollo_create_task", {
    title: "Create Task",
...
Returns:
  Created task data.`,
    ...
    if (params.contactId)
        body.contact_id = params.contactId;
    if (params.accountId)
        body.account_id = params.accountId;
    const response = await makeApiRequest("/tasks", "POST", body);
    if (!response.success) {
        return {
            content: [{ type: "text", text: `Error: ${response.error}` }],
        };
...
server.registerTool("apollo_search_tasks", {
    title: "Search Tasks",
...
Returns:
  List of matching tasks.`,
    ...
    if (params.contactIds)
        body.contact_ids = params.contactIds;
    if (params.accountIds)
        body.account_ids = params.accountIds;
    if (params.completed !== undefined)
        body.completed = params.completed;
    const response = await makeApiRequest("/tasks/search", "POST", body);
    if (!response.success) {
        return {
            content: [{ type: "text", text: `Error: ${response.error}` }],
        };
...
// DEAL TOOLS
// ============================================================
server.registerTool("apollo_create_deal", {
    title: "Create Deal",
...
Returns:
  Created deal data.`,
    ...
    if (params.ownerId)
        body.owner_id = params.ownerId;
    const response = await makeApiRequest("/opportunities", "POST", body);
    if (!response.success) {
        return {
            content: [{ type: "text", text: `Error: ${response.error}` }],
        };
...
server.registerTool("apollo_list_deals", {
    title: "List Deals",
...
Returns:
  Paginated list of deals.`,
    ...
    const response = await makeApiRequest("/opportunities", "GET", undefined, {
        page: params.page,
        per_page: params.perPage,
    });
    if (!response.success) {
...
// UTILITY TOOLS
// ============================================================
server.registerTool("apollo_get_api_usage", {
...
Returns:
  Current usage stats, rate limits, and remaining credits.`,
    ...
    const response = await makeApiRequest("/auth/health", "POST", {});
    if (!response.success) {
        return {
            content: [{ type: "text", text: `Error: ${response.error}` }],
        };
...
server.registerTool("apollo_list_users", {
    title: "List Users",
...
Returns:
  List of team members with IDs and roles.`,
    ...
    const response = await makeApiRequest("/users", "GET");
    if (!response.success) {
        return {
            content: [{ type: "text", text: `Error: ${response.error}` }],
        };
...
server.registerTool("apollo_list_email_accounts", {
...
Returns:
  List of email accounts with IDs for use in sequences.`,
    ...
    const response = await makeApiRequest("/email_accounts", "GET");
    if (!response.success) {
        return {
            content: [{ type: "text", text: `Error: ${response.error}` }],
        };
